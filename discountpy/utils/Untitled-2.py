from discountpy.utils.BitRepresentation import complementOne, _quadToByte, bytesToString, G
import numpy as np
from PyByteBuffer import ByteBuffer
class _NTBitArray:
    def encode(self, data: str) -> ZeroNTBitArray:
...
#   }
    ...
    def longsToString(self, data: np.array([], np.int64), offset: int, size: int) -> str:
        ...
        buf = bytearray(byte)
        self._longsToString(buf, sb, data, offset, size)

    """
    Optimised version for repeated calls - avoids allocating a new buffer each time
    """
    def _longsToString(self, buffer: bytearray, builder: str, data: np.array([], np.int64), offset: int, size: int) -> str:
        print(buffer, list(buffer), data, type(data))
        return bytesToString(buffer(data), builder, offset, size)
...
class NTBitArray(_NTBitArray):
    ...
    def slice(self, from: int, length: int) -> NTBitArray:
        return self.OffsetNTBitArray(self.data, from, length)
